...
Google käyttää OAuth-valtuutusta pienimmillä tarvittavilla kalenterien listaus- ja tapahtumien lukuoikeuksilla. State ja soveltuva PKCE, salatut refresh-tokenit, peruutuksen käsittely. OAuth-client ja Googlen hyväksymä redirect URI on testattava oikeassa kotiverkkoympäristössä: sisäistä `.home.arpa`-osoitetta ei oleteta automaattisesti kelvolliseksi. Tarvittaessa käytetään omistettua DNS-nimeä ja sisäistä DNS-ohjausta; kotiin ei avata porttia. Testitilan tokenien elinkaari ja suostumusnäytön julkaisuasetukset tarkistetaan ennen jatkuvaa käyttöä. [Googlen OAuth-ohje](https://developers.google.com/identity/protocols/oauth2/web-server)
...
Dokumentoitavat `.env.example`-kentät: APP_NAME, APP_ENV, APP_KEY, APP_DEBUG, APP_URL, APP_LOCALE=fi, APP_FALLBACK_LOCALE=fi, DB_CONNECTION=pgsql, DB_HOST, DB_PORT, DB_DATABASE, DB_USERNAME, DB_PASSWORD, SESSION_DRIVER=database, CACHE_STORE=database, QUEUE_CONNECTION=database, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI. APP_KEY ja salasanat jäävät mallissa tyhjiksi. Esimerkin toiminta-alueaikavyöhyke Europe/Helsinki on sovellusasetus; tallennusaika UTC. Foreca-/ENTSO-E-tunnukset syötetään hallinnassa ja salataan APP_KEY:llä.
